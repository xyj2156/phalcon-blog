...
class ArticlesController extends HomeBase
{
    ...
    public function indexAction ($id)
    {
        ...
        /** @var $visiCounter Counter */
        $visiCounter = $this->visitCounter;
        $visiCounter->calculate($post); // 访问量计算
        ...
        if (!$this->view->getCache()->exists('articles-'.$id)) {
            ...
            foreach ($postTermTaxonomy as $item) {
                if ($item->taxonomy == 'category') {
                    $term = $item->Terms;
                    if ($term) {
                        $taxonomy['category'][] = '<a href="'.$this->url->get([
                                'for' => 'index-category', 'params' => $term->slug,
                            ]).'" rel="category">'.$term->name.'</a>';
                    }
                } elseif ($item->taxonomy == 'tag') {
                    $term = $item->Terms;
                    if ($term) {
                        $taxonomy['tag'][] = '<a href="'.$this->url->get([
                                'for' => 'index-tag', 'params' => $term->slug,
                            ]).'" rel="tag">'.$term->name.'</a>';
                    }
            ...
            if ($last) {
                $lastHtml = '<a href="'.$this->url->get([
                        'for' => 'article', 'id' => $last['ID'],
                    ]).'" rel="prev">'.$last['post_title'].'</a>';
            } else {
                $lastHtml = '';
            }
            $next = $post->getNextPublish();
            if ($next) {
                $nextHtml = '<a href="'.$this->url->get([
                        'for' => 'article', 'id' => $next['ID'],
                    ]).'" rel="next">'.$next['post_title'].'</a>';
            } else {
                $nextHtml = '';
            }
...
//                $post = $post->toArray();
    ...
    public function slugAction ($alias = '')
    {
        if (empty($alias)) {
            return $this->response->redirect('/error/route404.html');
        }
        $post = Posts::findFirstByguid($alias);
        if (empty($post)) {
            return $this->response->redirect('/error/route404.html');
        }
        return $this->dispatcher->forward([
