...
namespace App\Library\Widget\Part\Taxonomy;


use App\Library\Widget\Part\Taxonomy;
    ...
    public function layout ($list, $pid = 0)
    {
        ...
        foreach ($list as $key => $value) {
            if ($value['parent'] == $pid) {
                ...
                $html .= '<li id="category-item-'.$value['term_taxonomy_id'].'" class="category-item"><a href="'.parent::$urlGenerator->get(['for'    => 'index-category',
                                                                                                                                             'params' => $value['slug'],
                    ]).'">'.$value['name'].'</a>';
